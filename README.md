...
本项目借助Trae 3.5.47（已更新到3.5.55版本，截至2026-05-06） 开发，使用了AI大模型。旨在于根据项目文件自动化生成“软件著作权申请材料”，包括合作开发协议、软著源码材料、软著用户手册、软著佐证材料、软著申请表甚至是演示视频的脚本和字幕。
...
> 部分 skill 的运行需要在本地配置好各种环境，例如 Tex Live 环境，包括安装 Tex Live 包、配置环境变量等。
...
- SC6_EXEPackager：软件从开发状态打包成可分发的exe文件（读取要求→打包成exe）
- SC7_ShowVideo：软件的样式视频点击脚本 + 字幕（读取模板→输出 Markdown以及生成点击脚本）
